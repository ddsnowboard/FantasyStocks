...
>Question: I was thinking of having an algorithm that would look at what you usually click on and automatically go to that. The problem is that it is possible that it would be annoying to load the page and have in muscle memory that you need to click on a certain tab, only to find out that it has already been clicked and you just went to a page that you didn't want to go to. This could be fixed by making sure that the tabs aren't re-ordered by the algorithm. 
